 ...
 * Doc PR 목록 — `#/doc-pr`
 *
 * `GET /doc-prs` (PR #107)로 목록을 받는다. 응답은 ID 중심이라
 * (`requesterId`/`approverId`, 이름 없음) 문서 제목만 `GET /documents`로 채우고
 * 사람은 ID로 표시한다 — 사용자 ID → 이름을 주는 엔드포인트가 아직 없다.
 *
 * 상태 필터와 검색은 받아 온 목록 위에서 처리한다 (서버 필터 파라미터 없음).
 */
...
const COLUMNS = [
    ...
    render: (row) => (
      <div className="min-w-0">
        ...
        <p className="mt-[2px] truncate text-[13px] text-neutral-500">
          {row.documentTitle} · 요청자 #{row.requesterId ?? "—"}
        </p>
      </div>
    ),
  },
  ...
  },
  {
    ...
    render: (row) =>
      row.approverId ? (
        <RaciChip role="A" name={`#${row.approverId}`} size="sm" />
      ) : (
  ...
  },
  {
    ...
    render: (row) =>
      row.mergedAt ? (
        <span className="flex items-center gap-[6px] text-[13px] text-neutral-500">
          {String(row.mergedAt).slice(0, 10)}
          {row.exceptionMerge && (
            <span
              title={row.exceptionReason ?? "예외 Merge"}
              className="rounded-full border border-warning/30 bg-warning-tint px-[6px] font-mono text-[10px] font-bold text-warning-text"
            >
              예외
            </span>
          )}
...
export default function DocPrListPage() {
  const { user } = useAuth();
  const { list, total, loading, error, reload } = useDocPrList(user.teamId);
  ...
  const rows = useMemo(() => {
    ...
    return list.filter((row) => {
      ...
      return (
        ...
        String(row.documentTitle).toLowerCase().includes(needle) ||
  ...
  return (
    <Page>
      <PageHeader
        ...
        properties={[
          { label: "전체", value: `${total}건` },
          {
            label: "승인권자 미지정",
            value: `${list.filter((row) => !row.approverId).length}건`,
          },
        ]}
      ...
      <div className="mt-[24px]">
        <ListFilterBar
          filters={[{ label: "상태", value: status, options: STATUS_OPTIONS, onChange: setStatus }]}
          searchLabel="Doc PR 검색"
          searchPlaceholder="제안 내용·문서명·번호 검색"
          value={keyword}
          onSearch={setKeyword}
        />
        <DataTable
